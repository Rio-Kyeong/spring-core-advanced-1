...
//    private TraceId traceIdHolder; // traceId 동기화, 동시성 이슈 발생
    ...
    public TraceStatus begin(String message) {
        syncTraceId();
        TraceId traceId = traceIdHolder.get(); // syncTraceId()를 수행하고 난 후이기 때문에 값이 설정되어 있다.
        // 시간 정보
    ...
    private void syncTraceId(){
        ...
        if(traceId == null){ // 최초 호출 시 초기 값 생성
    ...
    private void releaseTraceId() {
        TraceId traceId = traceIdHolder.get();
        if(traceId.isFirstLevel()){ // 만약 최초 호출 level == 0에 도달하면 내부에서 관리하는 traceId를 제거한다.
